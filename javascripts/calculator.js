...
document.addEventListener("DOMContentLoaded", calculator("#calculator1"));

function calculator(cssId) {
  // Select all buttons of the calculator and his screen
  var buttons = document.querySelectorAll(`${cssId} .calc-btn`);
  var screen = document.querySelector(`${cssId} output`);
   ...
   * @return {number}
   */
  function calculatorWrite(value) {
    if (value === "R") {
      // Reset all values
      screen.innerText = "0";
      calcValue = 0;
